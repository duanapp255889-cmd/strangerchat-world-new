...
export default function Home() {
  ...
  if (step === "matching") return <main className="shell matching-shell"><header className="topbar"><Logo compact /><LanguageToggle lang={lang} setLang={setLang} /></header><AdSlot label={lang === "vi" ? "Vị trí quảng cáo" : "Advertisement"} variant="top" /><section className="center-stage matching-stage"><div className="matching-orb"><div className="orb-ring ring-one" /><div className="orb-ring ring-two" /><div className="orb-core"><Sparkles size={29} /></div></div><h1>{c.matching}</h1><p>{c.matchingHint}</p><div className="matching-dots"><i /><i /><i /></div><button className="end-button" onClick={() => leaveChat(false)}><X size={15} />{c.end}</button></section><AdSlot label={lang === "vi" ? "Vị trí quảng cáo" : "Advertisement"} variant="bottom" /><LegalFooter lang={lang} onLegal={setLegalPage} />{legalPage && <LegalModal lang={lang} page={legalPage} onClose={() => setLegalPage(null)} />}</main>;
...
function AdSlot({ label, variant }: { label: string; variant: "top" | "bottom" | "chat" }) { return <div className={`ad-slot ad-${variant}`} aria-label={label}><a className="deal24h-ad" href="https://deal24h.net/?utm_source=external-site&utm_medium=banner&utm_campaign=deal24h-brand-awareness&utm_content=vertical-banner" target="_blank" rel="sponsored noopener noreferrer" aria-label="Visit Deal24h.net for coupons, promo codes and deals"><img src="https://deal24h.net/assets/ads/deal24h-banner-1161.webp" srcSet="https://deal24h.net/assets/ads/deal24h-banner-580.webp 580w, https://deal24h.net/assets/ads/deal24h-banner-1161.webp 1161w" sizes="(max-width: 640px) 100vw, 580px" width="1161" height="1355" alt="DEAL 24H — Big Brands, Real Discounts, All in One Place" loading="lazy" decoding="async" /></a></div>; }
