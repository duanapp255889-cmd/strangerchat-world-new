...
function Logo({ compact = false }: { compact?: boolean }) {
  return <div className={`flex items-center gap-3 ${compact ? "" : "flex-col gap-4"}`}>
    ...
    <span className={`brand ${compact ? "text-xl" : "text-3xl"}`}>Strangerly</span>
  </div>;
}
...
export default function Home() {
  ...
  const leaveChat = async (next: boolean) => {
    if (conversation) await supabase.from("conversations").update({ ended_at: new Date().toISOString() }).eq("id", conversation.id);
  ...
  if (step === "chat") return <ChatView c={c} lang={lang} session={session} messages={messages} draft={draft} setDraft={setDraft} sendMessage={sendMessage} leaveChat={leaveChat} />;
  if (step === "matching") return <main className="shell"><header className="topbar"><Logo compact /><LanguageToggle lang={lang} setLang={setLang} /></header><section className="center-stage"><div className="matching-orb"><div className="orb-ring ring-one" /><div className="orb-ring ring-two" /><div className="orb-core"><Sparkles size={29} /></div></div><h1>{c.matching}</h1><p>{c.matchingHint}</p><div className="matching-dots"><i /><i /><i /></div><button className="text-button" onClick={() => leaveChat(false)}>{c.end}</button></section></main>;

  return <main className="shell"><header className="topbar"><Logo compact /><LanguageToggle lang={lang} setLang={setLang} /></header><section className="center-stage wizard-stage">
  ...
  </section><footer className="footer"><span>© 2026 Strangerly</span><span>{lang === "vi" ? "Trò chuyện tử tế · Tôn trọng sự riêng tư" : "Be kind · Respect privacy"}</span></footer></main>;
}
...
function ChatView({ c, lang, session, messages, draft, setDraft, sendMessage, leaveChat }: { c: typeof copy.vi; lang: Language; session: SessionProfile | null; messages: Message[]; draft: string; setDraft: (value: string) => void; sendMessage: (event: FormEvent) => void; leaveChat: (next: boolean) => void }) { return <main className="chat-shell"><header className="chat-topbar"><Logo compact /><div className="chat-status"><span className="status-dot" /> {c.matched}</div><LanguageToggle lang={lang} setLang={() => undefined} /></header><section className="chat-card"><div className="chat-card-head"><div className="stranger-avatar"><Heart size={19} /></div><div><strong>{c.anonymous}</strong><span>{lang === "vi" ? "Một người ở đâu đó trên thế giới" : "Someone, somewhere in the world"}</span></div><button className="close-button" onClick={() => leaveChat(false)} aria-label={c.end}><X size={18} /></button></div><div className="messages"><div className="conversation-intro"><div className="intro-line" /><p>{c.empty}</p><div className="intro-line" /></div>{messages.map((message) => <div key={message.id} className={`message-row ${message.sender_session_id === session?.id ? "mine" : "theirs"}`}><div className="message-bubble">{message.body}</div><time>{new Date(message.created_at).toLocaleTimeString(lang === "vi" ? "vi-VN" : "en-US", { hour: "2-digit", minute: "2-digit" })}</time></div>)}</div><form className="composer" onSubmit={sendMessage}><input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder={c.placeholder} maxLength={2000} /><button type="submit" aria-label={c.send}><Send size={18} /></button></form></section><div className="chat-actions"><button className="secondary-button" onClick={() => leaveChat(true)}>{c.next}<span>↗</span></button><button className="text-button" onClick={() => leaveChat(false)}>{c.end}</button></div></main>; }
